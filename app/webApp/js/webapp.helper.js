 ...
 * */
/* eslint-disable */
import t from '../js/traduzione.js';
...
String.prototype.strpos = function strpos(needle,offset) {
    ...
    for (var i = offset; i < this.length+1-needle.length; i++){
...
if (typeof cerci_array != "function"){
  var cerci_array = function (matrix,search,striks){//Striks skifter fra if ( == ) til if ( === )
...
var mlinea_stato_trovato_funz = function(stato){return stato;};//Funzione, che viene esegito quando lo stato è trovato
var mlinea_appOnline = navigator.onLine; //Appens svar på, om man er online
...
function include_script(url,id,callbackFunc) {//Indsætter script på siden
  ...
  carica_file(url,id, function (testo_file) {
    if (ext == "css")
      appendElement("style",testo_file,document.head);
      ...
      mich.Ajax({
        ...
        onerror: function (motivazione,ajax_url){
          if (motivazione == "non accessibile")
            console.log(ajax_url+" non è accessibile");
...
var md = function (){
  ...
  function remove(key){
    ...
    if (ls_available === true){
      ...
      d.setTime(d.getTime() + (exdays * 24 * 60 * 60 * 1000));
      if (typeof value != "undefined")
        document.cookie = key + "=[removed]; expires=" + d.toUTCString() + "; path=/";
      return decodeURIComponent(document.cookie.replace(new RegExp("(?:(?:^|.*;)\\s*" + encodeURIComponent(key).replace(/[-.+*]/g, "\\$&") + "\\s*\\=\\s*([^;]*).*$)|^.*$"), "$1")) || null;
    }
  }
...
if (typeof m_el !== "function" && typeof HTMLElement !== "undefined") {
  var m_el = function (elemIdentifier,tipo){
...
export {mich, isset, isiOS, forceHTTPS, JSONparseNums, is_touch_device, preparato, senti, save2file, makeArray, esegui_scritti, include_script, carica_pagina, md, merge, debounce, clone, unico, elem, elems, bubbleTillClass, bubbleTillTag, moveCaretToEnd}